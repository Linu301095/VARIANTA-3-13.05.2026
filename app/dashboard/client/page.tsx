          ...
          <div style={{ textAlign: "center", maxWidth: 460, width: "100%" }}>
            ...
            <div style={{ background: "#fff", border: "2px solid #FF6B00", borderRadius: 20, padding: "20px 24px", marginBottom: 24, textAlign: "left" }}>
              {[["🏪 Salon", salon.nume], ["✂️ Serviciu", rezervare.serviciu], ["🕐 Ora", rezervare.ora], ["🐾 Animal", animal?.numeAnimal || "Animăluțul tău"]].map(([k, v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 14, padding: "8px 0", borderBottom: "1px solid #F3F4F6" }}>
                  <span style={{ color: "#6B7280" }}>{k}</span><span style={{ fontWeight: 700, color: "#1A1A1A" }}>{v}</span>
                </div>
              ))}
            </div>
    ...
    return (
      <Shell prenume={prenume} tab={tab} onLogout={() => router.push("/login")} onNav={setTab}>
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "28px 20px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: "24px", border: `2px solid ${salon.culoare}`, marginBottom: 20, boxShadow: "0 2px 20px rgba(0,0,0,.06)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              ...
              <div style={{ textAlign: "right" }}><div style={{ fontSize: 24, fontWeight: 900 }}>⭐ {salon.rating}</div><div style={{ fontSize: 12, color: "#9CA3AF" }}>{salon.recenzii} recenzii</div></div>
            </div>
            ...
            {[{ nume: "Tuns complet", pret: "80 RON", durata: "60 min" }, { nume: "Băiță + uscare", pret: "50 RON", durata: "40 min" }, { nume: "Tuns + Băiță + Unghii", pret: "120 RON", durata: "90 min" }, { nume: "Styling complet", pret: "150 RON", durata: "120 min" }].map(s => {
              ...
              return (
                <button key={s.nume} onClick={() => setRezervare(r => ({ ...r!, salonId: salon.id, serviciu: s.nume, ora: r?.ora || "" }))}
                  style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", borderRadius: 14, border: sel ? `2px solid ${salon.culoare}` : "1.5px solid #EBEBEB", background: sel ? salon.bg : "#fff", cursor: "pointer", fontFamily: "Nunito, sans-serif", textAlign: "left" }}>
                  <div><div style={{ fontSize: 14, fontWeight: 700, color: "#1A1A1A" }}>{s.nume}</div><div style={{ fontSize: 12, color: "#9CA3AF", marginTop: 2 }}>⏱ {s.durata}</div></div>
    ...
    <Shell prenume={prenume} tab={tab} onLogout={() => router.push("/login")} onNav={setTab}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: "clamp(20px,3vw,26px)", fontWeight: 900, color: "#1A1A1A", marginBottom: 10 }}>Bună, {prenume}! 🐾</h1>
          {animal && (
        ...
        {tab === "programari" && (
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 900, color: "#1A1A1A", marginBottom: 20 }}>📅 Programările mele</h2>
            {viitoare.length > 0 && (<>
            ...
            {trecute.length > 0 && (<>
              <div style={{ fontSize: 13, fontWeight: 800, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>Istoric</div>
        ...
        {tab === "notificari" && (
          <div style={{ maxWidth: 520 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "28px", border: "1.5px solid #EBEBEB", display: "flex", flexDirection: "column", gap: 0 }}>
              {[
                ...
                <div key={item.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 0", borderBottom: i < 2 ? "1px solid #F3F4F6" : "none" }}>
                  <div>
                  ...
                  <button onClick={() => setNotifSettings(n => ({ ...n, [item.key]: !n[item.key as keyof typeof n] }))}
                    style={{ width: 44, height: 24, borderRadius: 12, border: "none", background: notifSettings[item.key as keyof typeof notifSettings] ? "#FF6B00" : "#E5E7EB", cursor: "pointer", position: "relative", transition: "background .2s", flexShrink: 0 }}>
        ...
        {tab === "setari" && (
          <div style={{ maxWidth: 520 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "24px 28px", border: "1.5px solid #EBEBEB", marginTop: 16 }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: "#1A1A1A", marginBottom: 6 }}>Zona periculoasă</div>
              <div style={{ fontSize: 13, color: "#6B7280", marginBottom: 14 }}>Ștergerea contului este permanentă și nu poate fi anulată.</div>
              <button style={{ fontSize: 13, fontWeight: 700, color: "#EF4444", background: "#FEF2F2", border: "none", padding: "9px 18px", borderRadius: 50, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>Șterge contul</button>
            </div>
          </div>
        )}
        ...
        {tab === "ajutor" && (
          <div style={{ maxWidth: 620 }}>
            ...
            <div style={{ background: "#FFF3EA", border: "1px solid #FFDCC6", borderRadius: 16, padding: "18px 22px", marginTop: 20 }}>
              ...
              <div style={{ fontSize: 13, color: "#6B7280" }}>Contactează-ne la <strong>support@calyhub.ro</strong> sau prin chat live.</div>
            </div>
          </div>
        )}
        ...
        <div style={{ maxWidth: 1160, margin: "0 auto", padding: "0 20px", height: "100%", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setOpen(o => !o)}
              ...
              <span style={{ width: 30, height: 30, borderRadius: "50%", background: "#FFF3EA", border: "2px solid #FF6B00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>👤</span>
            ...
            {open && (
              <div style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, width: 262, background: "#fff", borderRadius: 18, border: "1.5px solid #EBEBEB", boxShadow: "0 8px 32px rgba(0,0,0,.12)", overflow: "hidden", zIndex: 200 }}>
                ...
                <div style={{ borderTop: "1px solid #EBEBEB", padding: "6px 0" }}>
                  <button onClick={() => { setOpen(false); onLogout(); }}
                    ...
                    <span style={{ width: 34, height: 34, borderRadius: 10, background: "#FEF2F2", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🚪</span>
      ...
      {items.map((item, i) => (
        <div key={i} style={{ background: "#fff", borderRadius: 14, border: open === i ? "2px solid #FF6B00" : "1.5px solid #EBEBEB", overflow: "hidden" }}>
          <button onClick={() => setOpen(open === i ? null : i)}
            style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", background: "none", border: "none", cursor: "pointer", fontFamily: "Nunito, sans-serif", textAlign: "left", gap: 12 }}>
...
function CardSalon({ salon, onSelect }: { salon: typeof SALOANE[0]; onSelect: () => void }) {
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1.5px solid #EBEBEB", overflow: "hidden", boxShadow: "0 2px 16px rgba(0,0,0,.05)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "18px 20px 20px", flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, fontWeight: 800, color: "#1A1A1A" }}>⭐ {salon.rating}<span style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>({salon.recenzii})</span></div>
        </div>
        <div><div style={{ fontSize: 17, fontWeight: 900, color: "#1A1A1A", marginBottom: 4 }}>{salon.nume}</div><div style={{ fontSize: 12, color: "#9CA3AF", fontWeight: 600 }}>📍 {salon.oras} · {salon.distanta}</div></div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto", paddingTop: 12, borderTop: "1px solid #F3F4F6" }}>
          <div><div style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 600 }}>de la</div><div style={{ fontSize: 18, fontWeight: 900, color: "#1A1A1A" }}>{salon.pretDe} RON</div></div>
    ...
    <div style={{ background: "#fff", borderRadius: 16, padding: "16px 20px", border: "1.5px solid #EBEBEB", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", boxShadow: "0 1px 8px rgba(0,0,0,.04)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
        ...
        {onAnuleaza && (p.status === "confirmat" || p.status === "în așteptare") && (
          <button onClick={() => onAnuleaza(p.id)} style={{ fontSize: 12, fontWeight: 700, color: "#EF4444", background: "#FEF2F2", border: "none", padding: "4px 12px", borderRadius: 50, cursor: "pointer", fontFamily: "Nunito, sans-serif" }}>Anulează</button>
        )}
      </div>
    </div>
  );
...
const btnSecondary: React.CSSProperties = { padding: "12px 24px", borderRadius: 50, border: "1.5px solid #EBEBEB", background: "#fff", color: "#374151", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Nunito, sans-serif" };
const btnBack: React.CSSProperties = { display: "flex", alignItems: "center", gap: 6, marginBottom: 20, background: "none", border: "none", fontSize: 14, fontWeight: 700, color: "#6B7280", cursor: "pointer", fontFamily: "Nunito, sans-serif", padding: 0 };
